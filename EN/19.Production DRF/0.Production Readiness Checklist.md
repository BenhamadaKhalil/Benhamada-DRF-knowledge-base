...
## 🛠️ Runbooks and operations

### 🚨 Emergency rollback plan

For every deployment target, document one rollback path:

```bash
# Example (adapt paths/env as needed)
...
- rollback command is written and tested in a staging environment
- migration reversibility is documented
- rollback reason is recorded in incident notes

### 👩‍💻 Incident response starter

Use this table for the first five minutes of a page:

- API latency spike: check slow-query logs and trace a sample request ID
- 5xx increase: inspect worker restarts and dependency health
- auth failures: verify auth classes, token table, and middleware config
...
- run auth + profile + create flows (`/user/token/`, `/user/me/`, `/api/recipe/`)
- verify data integrity with a small reconciliation query

### 📊 Logging + request-id checks

Define one query/read method for on-call:

- every request has a `request_id`
- every error includes that ID in logs
- every alert references the same ID, path, and user context

```text
{ "ts": "...", "level": "error", "path": "/api/recipe/", "status": 500,
  "request_id": "rq_1G8...", "user_id": 42, "method": "GET", "ms": 1200 }
```
