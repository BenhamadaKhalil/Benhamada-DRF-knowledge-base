...
> [!INFO] Django 6.1 renamed the low-level helper
> `transaction.savepoint()` is deprecated; use **`transaction.savepoint_create()`**. You rarely need it directly — a nested `atomic()` block is the idiomatic way to get a savepoint.
