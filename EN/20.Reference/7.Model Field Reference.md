...
| Value | When the parent is deleted | Handled by |
| --- | --- | --- |
| `CASCADE` | delete this row too | Django (Python) |
| `PROTECT` | **refuse** to delete the parent | Django |
| `SET_NULL` | set to `NULL` (requires `null=True`) | Django |
| `SET_DEFAULT` | set to the field's default | Django |
| `RESTRICT` | refuse, unless another cascade also removes it | Django |
| `DO_NOTHING` | leave a dangling FK — you're on your own | nobody |
| `DB_CASCADE` ✨ | delete this row too | **the database** |
| `DB_SET_NULL` ✨ | set to `NULL` | **the database** |
| `DB_SET_DEFAULT` ✨ | set to the default | **the database** |

`CASCADE` for owned data (a user's recipes). `PROTECT` for anything you'd hate to lose silently (orders referencing a product).

### ✨ The `DB_*` variants — new in Django 6.1
...
These emit a real `ON DELETE` clause in the schema instead of Django walking the object graph in Python. Dramatically faster when deleting a row with many children.

> [!CAUTION] `DB_*` does not fire signals
> No `pre_delete`, no `post_delete`. Anything you do in a delete signal stops happening, silently.
>
> In this project `Recipe.image` is cleaned up by a `post_delete` receiver. Switching that FK to `DB_CASCADE` would orphan every uploaded image on disk. See [[7.Signals]].
>
> Use `DB_CASCADE` for high-volume, side-effect-free relations. Keep `CASCADE` wherever a signal is doing real work.
